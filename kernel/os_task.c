...
INT8U  OSTaskSuspend (INT8U prio)
{
    ...
    if (prio == OS_PRIO_SELF) {                                 /* 挂起当前任务 ?*/
        prio = OSTCBCurrent->OSTCBPrio;
        self = TRUE;
    } else if (prio == OSTCBCurrent->OSTCBPrio) {               /* 挂起当前任务 ?*/
        self = TRUE;
    } else {
        self = FALSE;
    }
...
INT8U  OSTaskResume (INT8U prio)
{
    ...
    if ((ptcb->OSTCBStatus & OS_STAT_SUSPEND) != OS_STAT_RDY) { /* 任务被挂起 ?*/
        if (((ptcb->OSTCBStatus &= ~OS_STAT_SUSPEND) == OS_STAT_RDY) && /* 移除挂起状态*/
            (ptcb->OSTCBDelay == 0)) {                          /* 任务不能在等待时间*/
            OSReadyGroup                |= ptcb->OSTCBBit_Y;    /* 使任务就绪*/
            OSReadyTable[ptcb->OSTCB_Y] |= ptcb->OSTCBBit_X;
            OS_EXIT_CRITICAL();
            OS_Sched();                                         /* 还没定义OS_Sched*/
        } else {
            OS_EXIT_CRITICAL();
